...
**全站加速DCDN**支持纯动态加速和动静态混合加速

**SCDN**是阿里云提供的安全加速服务，它通过将您的内容分发到全球多个节点上，使用户能够快速地访问您的网站或应用程序，并且具有防御各种类型的网络攻击的能力。SCDN服务可以通过智能路由、智能缓存和HTTPS加密等技术，提供更高效、更稳定、更安全的内容分发服务。它可以帮助企业提升网站或应用程序的访问速度，提高用户体验，同时也可以有效地减轻源站的压力，降低带宽成本。
...
### CDN加速后出现空白页面
### 如何获取访客真实IP
* Windows平台下，如果使用IIS，需要安装一个“F5XForwardedFor”的扩展模块，才能在日志里看到“访客真实IP”。
* Apache和Nginx可以直接在Header中直接添加“X-Forwarded-For”，之后ECS可以直接从HTTP请求的Header中获取到。
                                            

# 日志
## 概念
* 日志服务（SLS）是原生观测分析平台，为**Log/Metirc/Tarce**等数据提供大规模、低成本、实时平台化服务。
* 一站式提供数据采集、加工、分析、警告可视化和投递功能。
...
2.  创建Project和Logstore
    * 项目Project是日志服务中的资源管理单元，用于资源隔离和控制。
    * 日志库Logstore是日志服务中日志数据的采集、存储和查询单元。日志服务提供标准型Standard和查询型Query两种类型的Logstore。
3.  采集日志
    * 接入数据
    * 安装Logtail，Logtail是日志服务提供的日志采集Agent
    * 创建机器组
    * 创建Logtail采集配置
4.  查询与分析日志
    * 配置索引后，可以查询和分析日志

<img src="./Iamge/LOGTAIL.png" width="600" />

## 日志的分析功能
* 标志性Logstore：支持日志服务一站式数据分析，适用于实时监控、交互式分析以及构建完整的可观测性系统等场景。
* 查询型Logstore：支持高性能查询，索引流量费用低于Standard，但不支持SQL分析，适合用于数据量大、存储周期长（周、月级别以上）、无日志分析场景。

如果用户要分析日志，则**必须将日志采集到Standard Logstore中**，且在配置索引时打开对应字段的**开启统计开关**。
## 日志的数据加工原理
* 日志服务的数据加工功能用协同消费组队源日志库的日志数据进行流式消费，**将每一条日志通过加工规则处理后再输出**。
* 每一个加工规则，加工服务的调度器会启动一个或者多个运行实例。每个运行实例扮演一个消费者的角色去消费一个或者多个Logstore的Shard.
...
|功能点|说明|
|----|----|
|告警抑制|用户可以添加抑制策略，用于阻止由某告警引发的其他警告通知。|
|告警静默|用户可以添加静默策略，在静默时间内，符合条件的告警，不会触发告警通知。|
|同一告警去重|时间窗口内，支持同一告警去重或者延迟发送。|
|告警分组合并|当系统出现大量重复的告警时，通过合并策略可将这些告警合并为一个告警进行通知。|
...
## 站点监控
站点监控可以对目标站点服务的可用性以及响应时间进行监控。系统已经默认预置了8种监控类型，包括**http监控、ping监控、tcp监控、udp监控、DNS监控、pop监控、smtp监控、ftp监控**。
## 报警联系人/组
* 删除报警联系人
* 删除报警联系组
* 删除报警联系组的报警联系人
